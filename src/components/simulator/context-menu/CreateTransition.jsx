...
function CreateTransition({ id, className }) {
  return (
    <div
      id={`transition-box-${id}`}
      className={cn(
        "absolute z-[200] flex translate-x-[-50%] translate-y-[-50%] flex-col items-center justify-center gap-2",
        className,
      )}
    >
      <div className="flex w-[220px] max-w-[220px] flex-col gap-2 rounded-md bg-main p-3 shadow-lg">
        ...
        <div className="flex w-full items-center justify-between">
          <div className="flex w-full items-center gap-1">
            <CustomDirectionButton direction="E" id={`transition-direction-left-${id}`} />
            <CustomDirectionButton direction="D" id={`transition-direction-right-${id}`} />
            <CustomDirectionButton direction="P" id={`transition-direction-stay-${id}`} />
          </div>
